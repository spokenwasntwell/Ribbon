...
export default class PokemonTCGCommand extends Command {
  ...
  public async run (msg: CommandoMessage, { props }: {props: Array<string>}) {
    startTyping(msg);
    const command = msg;
    ...
    if (props.includes('subtype')) {
      ...
      const subTypeSelection: any = await subTypePicker.obtain(command, [], 1);
    ...
    if (props.includes('supertype')) {
      ...
      const superTypeSelection: any = await superTypePicker.obtain(command, [], 1);
      ...
      if (poke.cards.length) {
        ...
        const cardChooser = new ArgumentCollector(command.client, [
          {
            ...
            type: 'integer',
            validate: (v: number) => v >= 1 && v <= 10 ? true : 'Please choose a number between 1 and 10',
            parse: (p: number) => p - 1,
          }
        ], 1);
        ...
        if (properties.supertype === 'pokemon') {
          ...
          attacks.forEach((item: any, attackIndex: any) => {
            tcgEmbed.addField(`Attack ${parseInt(attackIndex, 10) + 1}`, stripIndents`
              **Name:** ${item.name}
          ...
          if (resistances) {
            resistances.forEach((item: any, resistIndex: any) => {
              tcgEmbed.addField(`Resistance ${parseInt(resistIndex, 10) + 1}`, stripIndents`
              **Type:** ${item.type}
          ...
          if (weaknesses) {
            weaknesses.forEach((item: any, weaknessIndex: any) => {
              tcgEmbed.addField(`Weakness ${parseInt(weaknessIndex, 10) + 1}`, stripIndents`
              **Type:** ${item.type}
              **Multiplier:** ${item.value}
              `, true);
            });
