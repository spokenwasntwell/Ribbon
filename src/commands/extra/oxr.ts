  ...
  public async run (msg: CommandMessage, { value, fromCurrency, toCurrency }: {value: number, fromCurrency: string, toCurrency: string}) {
    try {
      ...
      oxrEmbed
        ...
        .addField(`:flag_${fromCurrency.slice(0, 2).toLowerCase()}: Money in ${fromCurrency}`, `${currencymap(fromCurrency) ? currencymap(fromCurrency) : ''}${value}`, true)
        .addField(`:flag_${toCurrency.slice(0, 2).toLowerCase()}: Money in ${toCurrency}`, `${currencymap(toCurrency) ? currencymap(fromCurrency) : ''}${result}`, true)
        .setTimestamp();
