...
export class Song {
  ...
  constructor (video: Video, member: GuildMember) {
    this.name = Util.escapeMarkdown(video.title);
    this.id = video.id;
    this.length = video.durationSeconds() ? video.durationSeconds() : video.duration.seconds / 1000;
    this.member = member;
